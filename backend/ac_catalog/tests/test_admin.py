...
def test_ac_model_changelist_renders(admin_client):
    ...
    resp = admin_client.get("/admin/ac_catalog/acmodel/")
...
def test_ac_model_add_renders(admin_client):
    resp = admin_client.get("/admin/ac_catalog/acmodel/add/")
    assert resp.status_code == 200
...
def test_ac_model_change_renders(admin_client):
    ...
    resp = admin_client.get(f"/admin/ac_catalog/acmodel/{m.pk}/change/")
    assert resp.status_code == 200


@pytest.mark.django_db
def test_ac_model_import_view_renders(admin_client):
    resp = admin_client.get("/admin/ac_catalog/acmodel/import-models/")
    assert resp.status_code == 200
...
def test_ac_model_import_template_xlsx_no_active_methodology_redirects(admin_client):
    ...
    resp = admin_client.get("/admin/ac_catalog/acmodel/import-template-xlsx/")
    assert resp.status_code == 302
    assert resp.url.endswith("/admin/ac_catalog/acmodel/")
...
def test_equipment_type_changelist(admin_client):
    resp = admin_client.get("/admin/ac_catalog/equipmenttype/")
    assert resp.status_code == 200
...
def test_brand_changelist(admin_client):
    ...
    resp = admin_client.get("/admin/ac_brands/brand/")
    assert resp.status_code == 200
...
def test_brand_origin_class_changelist(admin_client):
    resp = admin_client.get("/admin/ac_brands/brandoriginclass/")
    assert resp.status_code == 200
...
def test_review_changelist(admin_client):
    ReviewFactory()
    resp = admin_client.get("/admin/ac_reviews/review/")
    assert resp.status_code == 200
...
def test_submission_changelist(admin_client):
    ACSubmissionFactory()
    resp = admin_client.get("/admin/ac_submissions/acsubmission/")
    assert resp.status_code == 200
...
def test_calculation_run_changelist(admin_client):
    ...
    resp = admin_client.get("/admin/ac_scoring/calculationrun/")
    assert resp.status_code == 200


@pytest.mark.django_db
def test_admin_index_lists_all_ac_apps(admin_client):
    """Проверка что в /admin/ навигация содержит все 6 ac_* apps."""
    resp = admin_client.get("/admin/")
    assert resp.status_code == 200
    body = resp.content.decode().lower()
    # верхнего регистра в admin-навигации может не быть, ищем по нижнему
    for label in ("ac_brands", "ac_catalog", "ac_methodology", "ac_reviews", "ac_scoring", "ac_submissions"):
        assert label.replace("_", "-").lower() in body or label in body, f"app {label} не виден в админке"
