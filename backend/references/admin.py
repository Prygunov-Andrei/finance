...
from hvac_bridge.authentication import ServiceTokenAuthentication
from hvac_bridge.permissions import has_hvac_admin_access
...
def authenticate_jwt_request(request):
    ...
    """
    # Сначала пробуем service-token аутентификацию из ERP BFF.
    service_auth = ServiceTokenAuthentication()
    try:
        service_auth_result = service_auth.authenticate(request)
        if service_auth_result is not None:
            user, _token = service_auth_result
            logger.debug("Service token authentication successful for HVAC admin proxy")
            return user
    except Exception as e:
        logger.debug(f"Service token authentication failed: {str(e)}")
...
class ManufacturerAdmin(TranslationAdmin):
    ...
    def discover_manufacturers_news(self, request):
        ...
        if not has_hvac_admin_access(user, request.method):
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
