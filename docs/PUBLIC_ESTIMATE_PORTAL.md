...
**Фронтенд — отдельное React-приложение** в директории `portal/` корня проекта. Размещается на отдельном домене. Nginx на домене портала проксирует API-запросы на бэкенд.

```
┌──────────────────────────┐         ┌──────────────────────────┐
│   smeta-portal.ru        │         │   erp.example.com        │
│   (отдельный домен)      │         │   (текущий домен ERP)    │
│                          │         │                          │
│   React-приложение       │         │   React-приложение       │
│   (portal/)              │         │   (frontend/)            │
│   nginx раздаёт статику  │         │   nginx раздаёт статику  │
└──────────┬───────────────┘         └────────────┬─────────────┘
           │                                      │
...
Идентичный стек для единообразия и переиспользования компонентов:
...
```
portal/                              — Корень проекта (рядом с frontend/)
  Dockerfile
  nginx.conf                        — см. раздел 2.3
  package.json
  tsconfig.json
  vite.config.ts
  tailwind.config.js
  index.html
  src/
    App.tsx                          — Роутинг: / и /requests/:token
    main.tsx
    lib/
    ...
    components/
      ...
      estimate/
        EmailVerification.tsx        — Ввод email + OTP-код + кнопка "Получить код"
        FileUploadZone.tsx           — Drag-and-drop зона загрузки файлов
        ContactForm.tsx              — Название проекта + компания (email уже подтверждён)
        ...
        CallbackForm.tsx            — CTA: "Заказать оборудование" (телефон + комментарий)
      landing/
        HeroSection.tsx              — Главный баннер: "Рассчитайте смету за минуту"
        HowItWorks.tsx               — "Как это работает" (3 шага с иконками)
        UploadCTA.tsx                — Призыв к действию → скролл к форме
    hooks/
      useEstimateRequest.ts          — TanStack Query хуки для API
    pages/
      LandingPage.tsx                — Главная: Hero + HowItWorks + FileUpload + ContactForm
      RequestStatusPage.tsx          — /requests/:token — статус, прогресс, скачивание
    types/
      index.ts                       — TypeScript-типы (EstimateRequest, SpecificationItem, etc.)
    constants/
      index.ts                       — API_BASE_URL, статусы, сообщения
```
...
| `InvoiceService.recognize()` | `payments/services.py` | Парсинг счетов (для RFQ-ответов поставщиков, Фаза 3) |
...
> Выполнено. React SPA в portal/, Docker, nginx. Архитектура: раздел hvac-info.com/smeta/.

- [x] React-проект `portal/` (Vite + TypeScript + TanStack Query + Tailwind)
- [x] LandingPage:
...
- [x] Deployment guide: `portal/DEPLOY.md`
