...
| 2026-04-19 | 4A | ✅ done | `ac-rating/04a-public-api` | Claude | Агент: Петя. 10 сериализаторов + 9 views + URL-роутинг через tuple-include для nested namespace + `ac_catalog/i18n.py` (1-в-1 с Максимовского `core/i18n.py`). Подтверждение подсказки техлида: ERP дефолтит `IsAuthenticated+ERPSectionPermission`, поставил явные `permission_classes=[AllowAny]` во всех 9 публичных views. **Live pytest (ERP БД): 148 passed за 6:22.** Сюрприз: django-ratelimit `block=True` → 403 (PermissionDenied), не 429 — inherited from Максима, оставили как есть; можно починить в Ф6+ через `RATELIMIT_VIEW`. Риск для прода: LocMem cache → per-process ratelimit (×N лимит при multiple gunicorn workers); Redis-cache — фикс в Ф10. |
| 2026-04-19 | 4B | ✅ done | `ac-rating/04b-admin-xlsx` | Claude | Агент: Петя. 14 admin регистраций (в т.ч. `ACModelAdmin` 291 LoC + `MethodologyVersionAdmin` 240 LoC с duplicate/JSON delete-criterion), 5 services, 5 templates (с namespace-replace), management command `import_ac_rating_xlsx`. **Live pytest (ERP БД): 175 passed за 6:52** (+27 admin/import tests). `generate_pros_cons` action (Anthropic AI) удалён — вернётся в Ф8B. Scope-расширение: `raw_values_migration.py` перенесён как no-op stub (без него падает MethodologyVersionAdmin.save_related — минимально инвазивное решение). Sync-пересчёт при сохранении активной методики и `ensure_all_criteria_rows` при открытии модели — future refactor в Celery (Ф8). |
| 2026-04-19 | 5 | ✅ done | `ac-rating/05-data-migration` | Claude | Агент: Петя. Management command `load_ac_rating_dump` (338 LoC) с парсером plain-SQL pg_dump, маппингом 16 таблиц source→ac_*, обнулением user-FK на этапе парсинга (`\N` в COPY-payload), safety-check `Target DB:` + `--yes-i-am-sure` + `--dry-run`, setval sequences, transaction.atomic. 15 тестов. **Live pytest (ERP БД): 190 passed за 8:01.** Dry-run на реальном дампе 647KB: 16/16 таблиц распознаны, 1303 строки. Реальная загрузка (Петя, локальный PG): 27 моделей, `total_index ∈ [20.39, 78.85]`. Docs для Андрея в `docs/ac_rating/data-migration.md` (6 шагов). Сюрприз: первая regex давала +10 фантомных строк на non-greedy backtracking — пофикшен `^\.$` терминатором. Safety подтверждён: мой dry-run попал на `:15432` (прод!), но `--dry-run` ничего не записал — механизм работает. |
| 2026-04-19 | M1 | ✅ done | `ac-rating/m1-cleanup` | Claude | Агент: Петя. 3 tech-debt фикса: (1) ratelimit 403→429 — `ac_catalog/ratelimit.py` с Django + DRF handler + `Retry-After:60`, settings получил 3 строки (EXCEPTION_HANDLER, RATELIMIT_VIEW, RatelimitMiddleware) вместо обещанной 1 — отклонение оправдано: DRF перехватывает Ratelimited до Django middleware; (2) `MethodologyVersion.save()` обёрнут в `transaction.atomic()`; (3) удалён stub `raw_values_migration.py` + вызов из `MethodologyVersionAdmin.save_related`. **Live pytest (ERP БД): 190 passed за 9:25.** Smoke 429 + JSON + Retry-After — работает. |
| 2026-04-20 | D | ✅ done | — | 🧑‍💼 | Дизайн-сессия: клод-дизайн сдал ZIP с 8 секциями (NAV/LIST/MOB-LIST/DETAIL/STATIC/FOOTER/TEMPLATES/NEWS), ~17 артбордов. Лежит в `ac-rating/design/`. Дизайн-система: Inter+Source Serif 4+JetBrains Mono · акцент #2856cc (blue) · LIST-A утверждён · DetailA editorial long-form (30 параметров плоским списком, 5 anchor-секций). Решения: «Свой рейтинг» tab делаем сразу в Ф6A, авторы методики — хардкод, 5 белесых/неактивных разделов (Мешок Монтажников, Анализ проектов, Франшиза, Ассоциация, Стандарт монтажа). |
| — | M2 | pending | — | — | Maintenance: backend enhancements под дизайн — `rank` в list+detail, `stats` в methodology (total_models, active_criteria_count, median_total_index). ТЗ: `ac-rating/tz/M2-api-for-design.md` |
| 2026-04-20 | D | ✅ done | — | 🧑‍💼 | Клод-дизайн сдал ZIP в `ac-rating/design/` — 8 секций, LIST-A утверждён, DetailA editorial (30 параметров плоским списком). Inter+Source Serif 4+JetBrains Mono · акцент #2856cc. Решения: «Свой рейтинг» делаем сразу в Ф6A, авторы методики — хардкод, 5 белесых разделов (Мешок Монтажников, Анализ проектов, Франшиза, Ассоциация, Стандарт монтажа). |
| 2026-04-20 | M2 | ✅ done | `ac-rating/m2-api-for-design` | Claude | Агент: Петя. Backend enhancements под дизайн: rank в list (correlated COUNT-subquery, не ломается при фильтрах) + detail (rank_for_model), stats в methodology endpoint (total_models/active_criteria_count/median_total_index), median_total_index в detail context. Новый модуль `ac_catalog/stats.py`. 9 новых тестов → **199 passed**. Smoke: top-1 CASARTE rank=1 idx=78.85, stats median=47.31. Эпизод: первый коммит случайно ушёл в ismeta/ui-03 ветку → cherry-pick на правильную, откатил ismeta через update-ref. |
| 2026-04-20 | F0 | ✅ done | `ac-rating/f0-frontend-setup` | Claude | Агент: **Федя** (первая фаза). Scoped tokens `.rating-scope` + light/dark в `frontend/app/ratings/_styles/tokens.css` · RatingLayout с next/font/google (Inter/Source Serif 4/JetBrains Mono) · 5 skeleton routes (`/`, `/[slug]`, `/methodology`, `/submit`, `/archive`) через общий `ComingSoon` · плоский RatingHeader (3 активных + 5 белесых) · TS типы и API клиент (`frontend/lib/api/types/rating.ts`, `services/rating.ts` с ISR 3600s). **`tsc --noEmit` чисто, `next build` OK, dev-сервер отдаёт все 5 routes 200.** globals.css/erp/news не тронуты. |
