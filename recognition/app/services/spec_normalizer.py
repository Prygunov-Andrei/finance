...
NORMALIZE_PROMPT = """Ты обрабатываешь страницу проектной спецификации ОВиК (форма 1а ГОСТ 21.110).
...
5. Префикс-колонка «ПВ-ИТП». Если pos содержит короткий код системы (ПВ-ИТП,
   ...
   `name_final = f"{pos}-{name}"` если pos непуст. Пример: «ПВ-ИТП-Вентилятор
...
{
  "new_section": "<секция по окончании страницы, str>",
  "new_sticky": "<sticky parent name по окончании страницы, str>",
  "items": [
    {
      "name": "<str>",
      "model_name": "<str>",
      "brand": "<str>",
      "unit": "<str, например 'шт', 'м.п.', 'кг'>",
      "quantity": <number>,
      "comments": "<str>",
      "system_prefix": "<str, pos из row если был префикс системы>"
    }
  ]
}
...
sticky_parent_name: {sticky_parent_name!r}
rows: {rows_json}
"""
