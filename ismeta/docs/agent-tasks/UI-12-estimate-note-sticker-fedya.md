...
Твоя ветка: ismeta/ui-12-estimate-note-sticker
(создана от origin/main со всеми последними коммитами).

Текущий контекст: QA-цикл 10 заходов PO. Заход 1/10 закрыт,
идёт 2/10. Параллельно чистим backlog — это одна из задач.

Суть UI-12: стикер-заметка к смете. Одно жёлтое текстовое
поле (textarea) в шапке сметы с autosave-debounce, лимит
5000 символов. Никакой истории, свободно редактируется.
PO попросил дословно: «можно для красоты сделать жёлтым
листочком, типа стикера».

Зависимость: поле `Estimate.note` в API (TD-02 Пети).
**Проверь первым делом:** `git log --oneline main | grep TD-02`
— если TD-02 merged, стартуй. Если нет — жди, не начинай
(иначе не с чем работать).

Работай строго по ТЗ, не расширяй scope. В конце коммити в
свою ветку (git push origin ismeta/ui-12-estimate-note-sticker),
